    ...
    const pages = [
        <div key="account-info">
            <FormSection id="account-info" label="Account Information">
                ...
                <FormItem id="name" label="Company Name" value={editedCompany.name} required={true} />
                <FormItem id="description" label="Description" value={editedCompany.description} required={true} />
            </FormSection>
        </div>,
        <div key="contact-info">
            <FormSection id="contact-info" label="Contact Information">
                <FormItem id="address" label="Address" value={editedCompany.address} required={false} />
                <FormItem id="phone" label="Phone" value={editedCompany.phone} required={false} />
                <FormItem id="email" label="Email" value={editedCompany.email} required={true} />
                <FormItem id="website" label="Website" value={editedCompany.website} required={false} />
            </FormSection>
        </div>,
        <div key="social-media">
            <FormSection id="social-media" label="Social Media">
                <FormItem id="twitter" label="Twitter" value={editedCompany.social.twitter} required={false} />
                <FormItem id="facebook" label="Facebook" value={editedCompany.social.facebook} required={false} />
                <FormItem id="linkedin" label="LinkedIn" value={editedCompany.social.linkedin} required={false} />
            </FormSection>
        </div>
    ]
                            ...
                            <div id="form-buttons" className="flex justify-end space-x-2 mt-6">
                                ...
                                {currentPage === pages.length - 1 && (
                                    <>
                                        ...
                                        <Button id="save-button" type="submit" className="bg-[#F66C72] text-white hover:bg-opacity-90 transition duration-300">
                                            Save Changes
                                        </Button>
                                    </>
                                )}
